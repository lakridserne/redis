...
exports.createClient = function(options) {
  ...
  // Run a Cypher script against the server
  neo4jClient.runCypher = function(statement, callback) {
    neo4jClient.post('cypher', { query: statement }, callback);
  }
  ...
  neo4jClient.createNode = function(index, key, value, label, callback) {
    var input = {};
    input[key] = value;
    neo4jClient.post('node', input, function(obj) {
      ...
      neo4jClient.post(['node', obj.metadata.id, 'labels'], label, null);
    });
  }
  
  // lookup a node or create/index and cache it
  neo4jClient.lookupOrCreateNode = function(index, key, value, label, callback) {
    var
        ...
        if (!err && text) {
          ...
          neo4jClient.lookupNode(index, key, value, function(list, res) {
            if (list && list.length) {
              ...
              neo4jClient.createNode(index, key, value, label, function(obj) {
  ...
  neo4jClient.createRelationship = function(fromNodeId, toNode, type, callback) {
    neo4jClient.post(
      ['node', fromNodeId, 'relationships'], { to: toNode, type: type }, callback
    );
  }
